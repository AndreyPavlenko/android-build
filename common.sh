...
: ${ANDROID_TOOLCHAIN_ROOT:="$ANDROID_DEV_ROOT/toolchain/$ANDROID_NDK_ARCH-api$ANDROID_NDK_API"}
...
if [ ! -e "$ANDROID_TOOLCHAIN_ROOT" ]; then
  ...
  CMAKE="$CMAKE -DCMAKE_C_FLAGS=\"-O3 -DNDEBUG -flto -fvisibility=hidden -fdata-sections -ffunction-sections -Wno-unused-command-line-argument\""
  CMAKE="$CMAKE -DCMAKE_EXE_LINKER_FLAGS=\"-O3 -flto -Wl,--gc-sections -Wl,--strip-all\""
