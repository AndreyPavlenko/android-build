...
mkdir -p "$ANDROID_INSTALL_ROOT/include/libtransmission" "$ANDROID_INSTALL_ROOT/lib" "$ANDROID_INSTALL_ROOT/share"
cp -v $LIBS "$ANDROID_INSTALL_ROOT/lib"
cd ..
cp -v libtransmission/*.h "$ANDROID_INSTALL_ROOT/include/libtransmission"
cp -v build/libtransmission/*.h "$ANDROID_INSTALL_ROOT/include/libtransmission"
rsync -av --exclude='*.am' --exclude='*.in' --exclude='*.scss' web "$ANDROID_INSTALL_ROOT/share/transmission"

rm -f "$ANDROID_INSTALL_ROOT/share/transmission/web/checksum.sha1"
dir_checksum "$ANDROID_INSTALL_ROOT/share/transmission/web"\
           > "$ANDROID_INSTALL_ROOT/share/transmission/web/checksum.sha1"
